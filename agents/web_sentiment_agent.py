...
web_sentiment_agent = Agent(
    ...
    instruction=(
        ...
        "'people liked it'), in your own words, not verbatim quotes; and a "
        "one-sentence tone_consensus, or empty string if there isn't one. "
        "Only report opinions that are actually present in the search "
        "excerpts, never fill gaps with your own judgment. This data feeds "
        "sentiment_synthesis and the studio brief downstream — your job is "
        "specifically the raw reaction, not marketing tactics, not "
        "production facts, not competitive positioning, stay in your lane."
    ),
